...
# 예측모형 생성 및 설명 {#rf-hr-dataset-turnover-predictive-model-best}

<!-- http://rpubs.com/vishal1310/QuickIntroductiontoPartialDependencePlots -->

```{r rf-hr-dataset-variable-selection-fit}
library(randomForest)
library(ggRandomForests)
library(pdp)

churn_best_rf <- randomForest(churn ~ dependents + tenure + internet_service + online_security +
                              online_backup + tech_support + contract + monthly_charges +
                              total_charges, data = train_df, mtry=2)

churn_best_rf %>% 
  partial(pred.var = c("monthly_charges"), chull = TRUE, progress = "text", prob = TRUE) %>% 
  autoplot(contour = TRUE, rug = TRUE, pred.data = test_df)

gg_error(churn_best_rf) %>% plot

plot(gg_vimp(churn_best_rf))
